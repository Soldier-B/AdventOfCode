function AoC_4_2() {
	var rooms = [].slice.call(arguments);
	
	function Room(raw){  
		if(!(this instanceof Room)) return new Room(raw);
		var parts = raw.match(/^([-\w]+-)(\d+)\[(\w+)\]$/).splice(1,3);
		this.name = parts[0];
		this.sector = +parts[1];
		this.checksum = parts[2];    
	}
  
	Room.prototype.checksumValid = function(){
		var name = this.name.replace(/\W/g, ''), counts = [];
    
		while(name.length) {
			var char = name[0], len = name.length;
			name = name.replace(new RegExp(char, 'g'), '');
			counts.push({ char: char, count: len - name.length });
		}              
    
		return counts.sort(function(a, b){ return a.count == b.count ? (a.char.charCodeAt(0) - b.char.charCodeAt(0)) : (b.count - a.count); })
			.splice(0, 5)
			.map(function(c){ return c.char; })
			.join('') == this.checksum;
	};
  
	Room.prototype.decryptName = function(){
		var rot = this.sector % 26;
    
		return this.name.split('').map(function(char){
			return char == '-' ? ' ' : String.fromCharCode(((char.charCodeAt(0) - 97 + rot) % 26) + 97);
		}).join('').trim();
	};
  
	return rooms.map(Room).filter(function(room){   	
		return room.checksumValid() && room.decryptName() == 'northpole object storage'; 
	})[0].sector;    
}
