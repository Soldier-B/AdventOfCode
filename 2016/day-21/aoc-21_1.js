function AoC_21_1(inp, steps){
	
	return steps.reduce(function(data, step){
		step = step.split(' ');
	...
	function rotPos(x){ rotR.call(this, (x = this.indexOf(x)) + 1 + (x > 3 ? 1 : 0)); }
	function rev(x, y){ for(var z = (y - x + 1) >> 1; z--;) swapPos.call(this, x + z, y - z); }
	function move(x, y){ this.splice(y, 0, this.splice(x, 1)[0]); }
}

AoC_21_1('abcdefgh', ['swap letter e with letter h',
