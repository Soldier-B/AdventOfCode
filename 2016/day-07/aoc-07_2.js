function AoC_7_2(iplist){
	function ssl(ip){
		...
		return ip.split(/[\[\]]/g)
			.map(function(part, i){
				if(i % 2 == 0)
					for(var i = 0; i < part.length - 2; i++)
						if(part[i] == part[i + 2])
							aba.add(part.substr(i + 1, 2) + part[i + 1]);
				return part;
			})
			.filter(function(_, i){
				return i % 2;
			...
			.some(function(part){
				return Array.from(aba).some(function(aba){
					return part.indexOf(aba) + 1;
				});
			});
	}
	return iplist.filter(ssl).length;
}
