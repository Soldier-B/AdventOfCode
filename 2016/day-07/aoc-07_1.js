function AoC_7_1(iplist){
	function tls(ip){
		return ip.split(/[\[\]]/g)
			.map(function(part){ return /(\w)(?!\1)(\w)\2\1/.test(part); })
			.reduce(function(count, part, i){
				return count + (part ? (i % 2 ? NaN : 1) : 0);
			}, 0);
	}
	return iplist.filter(tls).length;
}
