...
function part1(bags) {
	return bags.reduce((c, b) => {
		return c + can_hold(b, 'shiny gold');
	}, 0);
}

function part2(bags) {
	return count_children(find_bag('shiny gold')) - 1;
}

function count_children(bag) {
	return 1 + bag.children.reduce((c, b) => {
		var count = count_children(find_bag(b.name))
		return c + b.qty * count;
	}, 0);
}

function find_bag(name) {
	return data.find(b => b.name == name);
}

function can_hold(bag, search) {
	var name = bag.name;

	if (!(name in cache))
		cache[name] = bag.children.some(c => {
			if (c.name == search) return true;
			return can_hold(find_bag(c.name), search);
		});

	return cache[name];
}

function parse_bag(row) {
	var data = row.replace('.', '').split(' contain '),
		bag = {
			name: get_bag_name(data[0]),
			children: []
		};

	if (data[1] != 'no other bags')
		for (var child of data[1].split(', ')) {
			var part = child.split(' ');
			bag.children.push({
				name: `${part[1]} ${part[2]}`,
				qty: +part[0]
			});
...
function get_bag_name(str) {
	var s = str.split(' ');
	return `${s[0]} ${s[1]}`;
}

function get_data() {
	return `posh brown bags contain 5 dim coral bags, 1 plaid blue bag, 2 faded bronze bags, 2 light black bags.
