var data = parse_groups(get_data().split('\n')),
	p1 = part1(data),
	p2 = part2(data);
...
function part1(groups) {
	return groups.reduce((c, g) => {
			var q = new Set();
			g.forEach(p => p.forEach(a => q.add(a)));
			return c + q.size;
		}, 0);
}
...
function part2(groups) {
	return groups.reduce((c, g) => {
		var q = {};
		g.forEach(p => p.forEach(a => { q[a] = (q[a] | 0) + 1; }));
		return c + Object.keys(q).filter(a => q[a] == g.length).length;
	}, 0);
}
