function aoc_4() {
	var records = Array.from(arguments).sort().map(Record),
		guards = [],
		id,
		guard,
		minute;
	...
	records.forEach(r => {
		switch (r.message) {
			case 'falls asleep':
				...
				break;
			case 'wakes up':
				var awake = r.timestamp.getMinutes();
				for(var i = minute; i < awake; i++)
					guard[i]++;
				break;
			default:
				id = +r.message.replace(/\D/g, '');
				guard = guards.filter(g => g.id == id)[0];
				if (!guard)
					guards.push(guard = Guard(id));
				break;
		}
	});

	// part 1
	guard = guards.sort((a, b) => b.totalSleep - a.totalSleep)[0],
	minute = guard.indexOf(Math.max.apply(0, guard));

	console.log('part 1: %i', guard.id * minute);

	// part 2 - which guard is most frequently alseep on the same minute
	guard = null;

	guards.forEach(g => {
		var max = Math.max.apply(0, g);

		if(!guard || max > guard[minute]){
			guard = g;
			minute = g.indexOf(max);
		}
	});

	console.log('part 2: %i', guard.id * minute);
...
function Guard(id) {
	var guard = Array(60).fill(0);
	guard.id = id;
	Object.defineProperty(guard, 'totalSleep', {
		get: function () {
...
aoc_4('[1518-10-31 00:58] wakes up',
