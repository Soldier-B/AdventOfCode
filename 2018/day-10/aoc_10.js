function aoc_10() {
    ...
    while (true) {
        if (points.every((p, i, a) => {
            for (var j = 0; j < a.length; j++) {
                if (i != j) {
                    if (Math.abs(p.x - a[j].x) < 2 && Math.abs(p.y - a[j].y) < 2)
                        return true;
                }
            }
            return false;
    ...
    var minx = Math.min.apply(0, points.map(p => p.x)),
        maxx = Math.max.apply(0, points.map(p => p.x)),
        miny = Math.min.apply(0, points.map(p => p.y)),
        maxy = Math.max.apply(0, points.map(p => p.y)),
        width = maxx - minx + 1,
        height = maxy - miny + 1,
        screen = Array(width * height).fill(' ');

    points.forEach(p => screen[(p.x - minx) + (p.y - miny) * width] = '#');

    // part 1
    for (var i = 0; i < height; i++)
        console.log(screen.slice(width * i, width * (i + 1)).join(''));

    // part 2
    console.log(time);
}
aoc_10('position=<-30509,  41062> velocity=< 3, -4>',
