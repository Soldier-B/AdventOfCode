function aoc_7_2(progs){
	progs = progs.reduce(function(a, b){
		var prog = {}
		
		b = b.split(' -> ');
		
		prog.name = b[0].match(/[a-z]+/);
		prog.weight = +b[0].match(/\d+/);
		prog.children = b.length == 1 ? [] : b[1].split(', ');
	...
	var tree = buildTree('mkxke'),
		pointer = tree,
		leaf,
		diff = 0;
	
	while(leaf = oddWeight(pointer))
		pointer = leaf;
	
	leaf = pointer.parent;
	
	diff = leaf.leaves.find(function(a){
		return a.weight != pointer.weight;
	}).weight - pointer.weight;
	
	return progs[pointer.name].weight + diff;
	
	function buildTree(head){
		var node = { name: head };
		
		node.leaves = progs[head].children.map(function(child){
			var childNode = buildTree(child);
			return (childNode.parent = node), childNode;
		});
		
		node.weight = progs[head].weight + node.leaves.reduce(function(a, b){ return a + b.weight; }, 0);
		
		return node;
	}
	
	function oddWeight(node){
		var weights = node.leaves.map(function(leaf){ return leaf.weight; }).sort();
		return node.leaves.find(function(leaf){ return leaf.weight != weights[1]; });
	}
}
