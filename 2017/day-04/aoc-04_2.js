function aoc_4_2(passphrases){
    return passphrases.filter(function(phrase){
        phrase = phrase.split(' ').map(function(str){ return str.split('').sort().join(''); }).join(' ');
        return !/(\b\w+\b).*(\b\1\b)/.test(phrase);
    }).length;
}
