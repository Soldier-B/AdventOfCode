...
function AoC_5_2(doorId){
	var i = 0;
	return doorId.split('').reduce(function(key){
		var hash;
		while(true){
			hash = Crypto.MD5(doorId + i++);
			if(hash.substr(0, 5) == '00000'){
				var pos = parseInt(hash[5], 16),
					char = hash[6];
				if(pos < 8 && typeof key[pos] == 'undefined'){
					key[pos] = char;
					break;
				}
			}
		...
		return key;
	}, Array.apply(0, Array(doorId.length))).join('');
}
