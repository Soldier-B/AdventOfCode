function aoc_6_2(instructions){
	var lights = {},
		exp = /(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)/
	
	instructions.forEach(function(instruction){
		var match = instruction.match(exp),
			action = match[1],
			x1 = +match[2],
			y1 = +match[3],
			x2 = +match[4],
		...
		for(var y = y1; y <= y2; y++)
			for(var x = x1; x <= x2; x++){
				var key = x + ',' + y,
					cur = lights[key],
					ton = action == 'turn on',
					toff = action == 'turn off',
					tog = action == 'toggle',
					brightness = ton ? 1 : toff ? -1 : tog ? 2 : 0;
				
				lights[key] = Math.max(0, (cur || 0) + brightness);
			}
	});
	
	return Object.keys(lights).reduce(function(count, key){
		return count + +lights[key];
	}, 0);
}
