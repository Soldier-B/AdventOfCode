function aoc_16(){
	var aunts = arguments[0].map(parse),
		detect = {
			children: 3,
			cats: 7,
			samoyeds: 2,
	...
	part1 = aunts.filter(function(a){
		return Object.keys(detect).every(function(key){
			return key in a ? detect[key] == a[key] : true;
		});
	})[0].num;
	
	part2 = aunts.filter(function(a){
		return Object.keys(detect).every(function(key){
			if(!(key in a)) return true;
			
			if(/cats|trees/.test(key))
				return a[key] > detect[key];
			else if (/goldfish|pomeranians/.test(key))
				return a[key] < detect[key];
			
			return detect[key] == a[key];
		});
	})[0].num;
